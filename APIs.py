...
def read_one(person_id):
    ...
    person = db.session.query(Person).filter(Person.id == person_id).one_or_none()
...
def create(person):
    ...
    existing_person = (
        db.session.query(Person).filter(Person.fname == fname)
...
def update(person_id, person):
    ...
    update_person = db.session.query(Person).filter(
        Person.id == person_id
    ).one_or_none()
    ...
    # Would our update create a duplicate of another person already existing?
        ...
        existing_person is not None and existing_person.id != person_id
    ):
        abort(
            409,
            "Person {fname} {lname} exists already".format(
